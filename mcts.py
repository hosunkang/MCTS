...
class standard_MCTS:
    ...
    def backprop(self, result, nd):
        ...
        if nd.parentND is not None:
            nd.parentND = self.backprop(result, nd.parentND)
            for child in nd.parentND.childNDs:
                child.utc = self.utcFunc(child.val, child.vis, child.parentND.vis)
        return nd
